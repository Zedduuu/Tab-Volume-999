...
async function ensureOffscreenDocument() {
  ...
  try {
    await chrome.offscreen.createDocument({
      ...
      justification: '回放标签页被接管的音频，并通过 Web Audio 增益节点实现 0%~200% 音量调节。',
    });
  } catch (err) {
...
async function handleStart({ tabId }) {
  if (typeof tabId !== 'number') return { ok: false, error: '参数无效' };
...
async function handleCapture({ tabId, streamId, volume }) {
  if (typeof tabId !== 'number' || typeof streamId !== 'string') {
    return { ok: false, error: '参数无效：缺少 tabId 或 streamId' };
  }
  await ensureOffscreenDocument();
...
async function recaptureTab(tabId, attempt) {
  ...
  try {
    ...
    if (!resp?.ok) throw new Error(resp?.error || '重新接管失败');
